...
import org.springframework.security.oauth2.core.endpoint.OAuth2AlipayMiniProgramParameterNames;
import org.springframework.security.oauth2.core.endpoint.OAuth2ParameterNames;
import org.springframework.security.oauth2.server.authorization.authentication.OAuth2AlipayMiniProgramAuthenticationToken;
...
/**
 * 微信 OAuth2 用于验证授权授予的 {@link OAuth2AlipayMiniProgramAuthenticationToken}。
 *
 * @author xuxiaowei
 * @since Joe Grandja
 * @since 0.0.1
 ...
 */
public class OAuth2AlipayMiniProgramAuthenticationConverter implements AuthenticationConverter {
	...
	public Authentication convert(HttpServletRequest request) {
		...
		if (!OAuth2AlipayMiniProgramAuthenticationToken.ALIPAY_MINIPROGRAM.getValue().equals(grantType)) {
			return null;
		}
		...
		if (!StringUtils.hasText(code) || parameters.get(OAuth2ParameterNames.CODE).size() != 1) {
			OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2ParameterNames.CODE,
					OAuth2AlipayMiniProgramEndpointUtils.AUTH_ALIPAY_SYSTEM_OAUTH_TOKEN_URI);
		}

		// appid (REQUIRED)
		String appid = parameters.getFirst(OAuth2AlipayMiniProgramParameterNames.APPID);
		if (!StringUtils.hasText(appid)) {
			...
			while (headers.hasMoreElements()) {
				...
				if (StringUtils.hasText(nextElement)) {
					...
					if (split.length > 3) {
						appid = split[3];
						parameters.add(OAuth2AlipayMiniProgramParameterNames.APPID, appid);
					}
				}
			}
		}
		if (!StringUtils.hasText(appid) || parameters.get(OAuth2AlipayMiniProgramParameterNames.APPID).size() != 1) {
			OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST,
					OAuth2AlipayMiniProgramParameterNames.APPID,
					OAuth2AlipayMiniProgramEndpointUtils.AUTH_ALIPAY_SYSTEM_OAUTH_TOKEN_URI);
		}
		...
		parameters.forEach((key, value) -> {
			if (!key.equals(OAuth2ParameterNames.GRANT_TYPE) && !key.equals(OAuth2ParameterNames.CLIENT_ID)
					...
					&& !key.equals(OAuth2AlipayMiniProgramParameterNames.APPID)
					&& !key.equals(OAuth2ParameterNames.SCOPE)) {
				additionalParameters.put(key, value.get(0));
			}
		});

		return new OAuth2AlipayMiniProgramAuthenticationToken(clientPrincipal, additionalParameters, appid, code,
				scope);
	}
